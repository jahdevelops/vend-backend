...
exports.logout = catchAsyncErrors(async(req, res, next) => {
    ...
    mysql.query(findUserById, [userId], async(err, data) => {
        ...
        mysql.query(
            findToken, [data[0].id, "refresh_token"],
            async(err, token) => {
                if (err) return next(new ErrorHandler(err.message, 500));
                if (!token.length) {
                    return next(
                    ...
                    for (const Rtoken of token) {
                        ...
                        if (isValid) {
                            tokenExists = true;
                            mysql.query(
                                deleteTokenOne, [data[0].id, "refresh_token", Rtoken.id],
                                (err) => {
                                    if (err) return next(new ErrorHandler(err.message, 500));
                                }
                            );
    ...
    mysql.query(findUserById, [userId], async(err, data) => {
        ...
        mysql.query(findToken, [userId, "refresh_token"], async(err, token) => {
            ...
            const accessToken = jwt.sign({
                ...
                jwt_secret, { expiresIn: "30 min" }
            );
            refreshToken = crypto.randomBytes(32).toString("hex");
            const hash = await bcrypt.hash(refreshToken, 10);

            const refreshTokenJWTNew = jwt.sign({ userId, refreshToken },
                jwt_secret, {
                    expiresIn: "1 day",
                }
            );
...
exports.requestPasswordReset = catchAsyncErrors(async(req, res, next) => {
    ...
    mysql.query(findUserByEmail, [email], async(err, data) => {
        ...
        mysql.query(
            findToken, [data[0].id, "reset_password"],
            async(err, token) => {
                if (err) return next(new ErrorHandler(err.message, 500));
...
const generateAuthToken = async(userId, role, isVerified) => {
    const accessToken = jwt.sign({ id: userId, role: role, isVerified: isVerified },
        jwt_secret, { expiresIn: "30 min" }
    );
